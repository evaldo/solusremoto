<?php
	...
	if(isset($_POST['botaoconsultar'])&& $_POST['textoconsulta']<>""){
		...
		$sql ="SELECT id_status_trtmto, ds_status_trtmto, case when fl_ativo=1 then 'Sim' else 'Não' end fl_ativo, cd_cor_status_trtmto 
				from tratamento.tb_c_status_trtmto 
				where upper(ds_status_trtmto) like '%" . $textoconsulta . "%' order by cd_cor_status_trtmto LIMIT $itens_por_pagina OFFSET $pagina*$itens_por_pagina";
			...
			$sql ="SELECT id_status_trtmto, ds_status_trtmto, case when fl_ativo=1 then 'Sim' else 'Não' end fl_ativo, cd_cor_status_trtmto from tratamento.tb_c_status_trtmto order by cd_cor_status_trtmto LIMIT $itens_por_pagina OFFSET $pagina*$itens_por_pagina";	
	}
	...
	if(isset($_POST['insere'])){					
		...
		{	
			...
			$sql = "insert into tratamento.tb_c_status_trtmto (id_status_trtmto, ds_status_trtmto, fl_ativo, cd_cor_status_trtmto, cd_usua_incs, dt_incs) values ((select NEXTVAL('tratamento.sq_status_trtmto')), '". $_POST['ds_status_trtmto']."', ". $_POST['fl_ativo'].", '". $_POST['cd_cor_status_trtmto']."', '".$_SESSION['usuario']."', current_timestamp);";
	...
	if(isset($_POST['altera'])){					
		...
		{	
			...
			$sql = "update tratamento.tb_c_status_trtmto set ds_status_trtmto = '". $_POST['ds_status_trtmto']."', fl_ativo = ". $_POST['fl_ativo'].", cd_usua_altr = '".$_SESSION['usuario']."', cd_cor_status_trtmto = '". $_POST['cd_cor_status_trtmto']."',  dt_altr = current_timestamp where id_status_trtmto = ". $_SESSION['id_status_trtmto']."";	
...
?>	
	...
	<head>
	 ...
	 <title>Cadastro dos Status de Tratamento</title>
	 ...
	 <div id="main" class="container-fluid" style="margin-top: 50px"> 
		<div class="container" style="margin-left: 0px">
			<form class="form-inline" action="#" method="post" >				
				<b>Consultar Status de Tratamentos:</b>:&nbsp;&nbsp													
		...
		<div class="table-responsive col-md-12">
			<table class="table table-striped" cellspacing="0" cellpadding="0" id="tabela">
				<thead>
					<tr>
						<th>Identificador do Status de Tratamento</th>
						<th>Descrição do Status de Tratamento</th>
						<th>Flag Ativo?</th>	
						<th>Cor no Painel</th>	
				...
				<?php
					...
					?>						
						<tr>
							...
							<td id="ds_status_trtmto" value="<?php echo $row[1];?>"><?php echo $row[1];?></td>
							<td id="fl_ativo" value="<?php echo $row[2];?>"><?php echo $row[2];?></td>
							<td id="cd_cor_status_trtmto" value="<?php echo $row[3];?>"><?php echo $row[3];?></td>
	...
	$(document).ready(function(){
		...
		$("#tabela").on('click','.delecao',function(){			
			...
			var ds_status_trtmto = currentRow.find("td:eq(1)").text();	
			var fl_ativo = currentRow.find("td:eq(2)").text();
			var cd_cor_status_trtmto = currentRow.find("td:eq(3)").text();	
			
			// AJAX code to submit form.
			$.ajax({
				 type: "POST",
				 url: "../delecao/delecao_status_tratamento.php", //
				 data: {id_status_trtmto:id_status_trtmto, ds_status_trtmto:ds_status_trtmto, fl_ativo:fl_ativo, cd_cor_status_trtmto:cd_cor_status_trtmto},
				 dataType : "text",			 
