<?php
	...
	if(isset($_POST['botaoconsultar'])&& $_POST['textoconsulta']<>""){
		...
		$sql ="SELECT cd_usua_acesso, nm_usua_acesso 
			...
			$sql ="SELECT cd_usua_acesso, nm_usua_acesso from pts.tb_c_usua_acesso order by nm_usua_acesso LIMIT $itens_por_pagina OFFSET $pagina*$itens_por_pagina";	
	}
	...
	if(isset($_POST['insere'])){					
		...
		{	
			...
			$sql = "insert into pts.tb_c_usua_acesso (cd_usua_acesso, nm_usua_acesso, ds_usua_acesso, fl_sist_admn, cd_faixa_ip_1, cd_faixa_ip_2, fl_acesso_ip, cd_usua_incs, dt_incs) values ((select NEXTVAL('pts.sq_usua_acesso')), '". $_POST['nm_usua_acesso']."', '". $_POST['ds_usua_acesso']."' ,'". $fl_sist_admn ."','". $_POST['cd_faixa_ip_1']."', '". $_POST['cd_faixa_ip_2']."','". $fl_acesso_ip ."', '".$_SESSION['usuario']."', current_timestamp);";
	...
	if(isset($_POST['deleta'])){					
		...
		try
		{
			...
			if ($row[0]>0){
				...
				// remove do banco			
				$sql = "DELETE FROM pts.tb_c_usua_acesso WHERE cd_usua_acesso = ".$_SESSION['cd_usua_acesso']."";			
				$result = pg_query($pdo, $sql);
