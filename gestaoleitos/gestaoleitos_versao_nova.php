<?php
	...
	//Lista leitos com pacientes admitidos e carregados do Smart com Leitos vazios no Integração Gestão de Leitos
	...
	while($rowSmart = pg_fetch_row($retSmart)) {
		...
		//Verifica se existe o leito carregado do Smart na base do Integração do Gestão de Leitos
		...
		//Se não existir, insere o leito do Smart no Integração Gestão de Leitos
		//Caso contrário, atualiza os dados do Leito no Integração Gestão de Leitos
		$data_prevista_alta = $rowSmart[2];	
		
		if ($rowSmart[2]==null){			
			$data_prevista_alta = '01/01/1900';				
		}
		
		if ($rowCtrlLeito[0]==0){
			
			$sql = "INSERT INTO integracao.tb_ctrl_leito (cd_ctrl_leito, DS_LEITO, DS_ANDAR, DT_PRVS_ALTA, NM_PCNT, DS_SEXO, DT_NASC_PCNT, NM_CNVO, PAC_REG, DT_ADMSS, id_memb_equip_hosptr_mdco, nm_mdco) VALUES ((SELECT MAX(cd_ctrl_leito)+1 FROM integracao.tb_ctrl_leito), '" . $rowSmart[0] . "', '" . $rowSmart[1] . "', '" . $data_prevista_alta . "', '" . $rowSmart[3] . "', '" . $rowSmart[4] . "', '" . $rowSmart[5] . "', '" . $rowSmart[6] . "', " .$rowSmart[7] . ", '" . $rowSmart[8] . "', " .$rowSmart[9] . ", '".$rowSmart[10]."')";	
			...
			$sql = "UPDATE integracao.tb_ctrl_leito SET pac_reg = " . $rowSmart[7] . ", DS_ANDAR = '" . $rowSmart[1] . "', DT_PRVS_ALTA = '" . $data_prevista_alta . "', NM_PCNT = '" . $rowSmart[3] . "', DS_SEXO = '" . $rowSmart[4] . "', DT_NASC_PCNT = '" . $rowSmart[5] . "', NM_CNVO = '" . $rowSmart[6] . "', DT_ADMSS = '" .$rowSmart[8] . "' , id_memb_equip_hosptr_mdco = " .$rowSmart[9] . ", nm_mdco =  '".$rowSmart[10]."' WHERE trim(DS_LEITO) = '" . $rowSmart[0] . "' ";
			...
			//Caso o leito exista no Integração do Gestão de Leitos Web, os dados da tabela TB_CTRL_LEITO são 
			//atualizados pela tabela temp de leitos, TB_CTRL_LEITO_TEMP
	...
	//Caso o leito não tenha mais o paciente, vide view vw_ctrl_leito, com o código de paciente igual a 0,
	//atribuir null para os campos de gestão de leito para as tabelas TB_CTRL_LEITO e TB_CTRL_LEITO_TEMP
	...
	//Atualizar os dados da tabela TB_CTRL_LEITO para a tabela TB_CTRL_LEITO_TEMP
	...
	//Verifica permissão de transação
	$atualizadadospcnt="";
	...
	$sql ="select  
		...
		ds_dieta,
		ds_const,
		case when dt_prvs_alta = '01/01/1900' then '' else to_char(dt_prvs_alta, 'dd/mm/yyyy hh24:mi') end as dt_prvs_alta,
